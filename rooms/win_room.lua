WinRoom = Object:extend()

function WinRoom:new()
    self.t = 0
    self.y = 100
    self.text_y = 0
    ...
    assets.audios[AUDIO_ID.WIN]:setVolume(GAME_DATA.AUDIO_VOLUME)
end

function WinRoom:update(dt)
    self.t = self.t + dt
    self.text_y = math.sin(self.t * 3) * 8
    self.timer:update(dt)
    self.anim8:update(dt)
    if input:pressed('accept_action') then
        assets.audios[AUDIO_ID.WIN]:stop()
        go_to_room('TitleRoom')
    end
end

function WinRoom:draw()
    self:draw_title()
...
function WinRoom:draw_continue_text()
